...
export default function AddressPage() {
  ...
  return (
    <Container className="bg-[var(--color-background)] relative w-[390px]">
      {/* 강조 배경 */}
      <div className="absolute top-[60px] left-[90px] w-[80px] h-[80px] round-full bg-[radial-gradient(ellipse_at_center,_var(--color-orange)_0%,_transparent_70%)] z-0" />

      {/* 상단 문구 */}
      <div className="pt-[80px] px-6 z-10">
      ...
      {/* 입력창 및 위치 설정 */}
      <div className="px-6 pt-8 space-y-0 z-10">
        <PillTextInputRoot>
          <PillTextInput
        ...
        {inputValue.trim() === '' && (
          <div className="flex justify-end pt-0">
            <PillButton
              variant="link"
              size="sm"
              className="text-[12px] font-medium text-[var(--color-orange)] hover:text-[var(--color-orange)]"
            >
        ...
        {inputValue.trim() !== '' && (
          <div className="pt-4 space-y-3 text-sm pl-4">
            ...
            <div>
              ...
              <p className="text-[var(--color-orange)] font-medium">역삼로 180</p>
            </div>
            <div>
              <p className="text-secondary">지역명(동/리)+번지</p>
              <p className="text-[var(--color-orange)] font-medium">강남구 역삼동 790-6</p>
            </div>
            <div>
              <p className="text-secondary">지역명(동/리)+건물명</p>
              <p className="text-[var(--color-orange)] font-medium">역삼동 마루 180</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="px-6 pb-12 pt-8 z-10 mt-auto">
        <PillButton
          ...
          className="bg-[var(--color-orange)] hover:bg-[var(--color-orange)] text-black font-bold"
        >
          등록하기
        </PillButton>
      </div>
      ...
      {isRegistered && (
        <div className="absolute inset-0 bg-black/30 backdrop-blur-[2px] flex items-center justify-center z-50">
          <div className="bg-white px-6 py-4 rounded-xl flex items-center gap-4 shadow-lg">
            <CheckIcon className="text-[var(--color-primary-bg)]" />
